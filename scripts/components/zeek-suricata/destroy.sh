#!/usr/bin/env bash
# scripts/components/zeek-suricata/destroy.sh
...
STATE_FILE="${SOC_STATE_DIR}/state/zeek-suricata.json"
log() { printf '[zs-destroy] %s\n' "$*"; }
...
rm -f "${STATE_FILE}"
log "zeek-suricata teardown complete (VMID ${VMID})"
