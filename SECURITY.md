...
- **MCP servers bind to 127.0.0.1 by default.** The SSE endpoints carry bearer tokens with admin-level reach into every component. `--mcp-bind-host 0.0.0.0` exposes them; only do that on a network where you trust every host. `/root/mcp-clients.json` contains the bearer tokens and is written `0600`.
