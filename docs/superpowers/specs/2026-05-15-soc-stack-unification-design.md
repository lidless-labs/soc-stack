...
| `--ip-range` | - | required if `--ip-mode=static` (e.g., `10.0.50.10/24`) |
...
2. On success or already-deployed, writes `${SOC_STATE_DIR}/state/<name>.json`:
   ```json
   {
     "component": "wazuh",
     "status": "deployed",
     "url": "https://10.0.50.10",
     "api_url": "https://10.0.50.10:55000",
  ...
  "components": [
    {
      "name": "wazuh",
      "status": "deployed",
      "lxc": { "vmid": 201, "hostname": "s3-wazuh", "ip": "10.0.50.10" },
      "preset": "standard",
      "ports": [443, 1514, 1515, 55000],
      "endpoints": {
        "dashboard": "https://10.0.50.10",
        "api": "https://10.0.50.10:55000",
        "agent_enrollment": "10.0.50.10:1515"
      },
      "credentials": {
        "admin_user": "admin",
...
```json
{
  ...
  "mcpServers": {
    "wazuh":    { "type": "sse", "url": "http://10.0.50.99:3001/sse", "headers": { "Authorization": "Bearer <token>" } },
    "thehive":  { "type": "sse", "url": "http://10.0.50.99:3002/sse", "headers": { "Authorization": "Bearer <token>" } },
    "cortex":   { "type": "sse", "url": "http://10.0.50.99:3003/sse", "headers": { "Authorization": "Bearer <token>" } },
    "misp":     { "type": "sse", "url": "http://10.0.50.99:3004/sse", "headers": { "Authorization": "Bearer <token>" } },
    "zeek":     { "type": "sse", "url": "http://10.0.50.99:3005/sse", "headers": { "Authorization": "Bearer <token>" } },
    "suricata": { "type": "sse", "url": "http://10.0.50.99:3006/sse", "headers": { "Authorization": "Bearer <token>" } },
    "mitre":    { "type": "sse", "url": "http://10.0.50.99:3007/sse", "headers": { "Authorization": "Bearer <token>" } },
    "rapid7":   { "type": "sse", "url": "http://10.0.50.99:3008/sse", "headers": { "Authorization": "Bearer <token>" } },
    "sophos":   { "type": "sse", "url": "http://10.0.50.99:3009/sse", "headers": { "Authorization": "Bearer <token>" } }
  },
  "raw_endpoints": [
    { "name": "wazuh-mcp", "url": "http://10.0.50.99:3001/sse", "token": "<token>" }
  ]
}
```
